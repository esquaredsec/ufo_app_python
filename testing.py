...
# Extract the links from the table (first 6 links)
links = [link['href'] for link in table.find_all('a')[:6]]

# Initialize an empty list to store UFO sightings data
all_ufo_sightings = []
...
for link in links:
    ...
    # Initialize an empty list to store UFO sightings data for this link
    ufo_sightings = []

    # Loop through each row in the table
    for row in sub_table.find_all('tr')[1:]:  # Skip the header row
        columns = row.find_all('td')

        # Ensure that there are at least 3 columns in the row before extracting data
        if len(columns) >= 8:  # Adjusted to check for at least 8 columns
            date = columns[7].text.strip()
            # Join columns 2, 3, and 4 and separate them with commas
            location_parts = [columns[1].text.strip(), columns[2].text.strip(), columns[3].text.strip()]
